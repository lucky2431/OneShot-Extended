...
class WiFiCollector:
    ...
    def addNetwork(self, bssid: str, essid: str, wpa_psk: str) -> bool:
        ...
        try:
            ...
            if src.utils.isAndroid() is True:
                ...
                result = subprocess.run(android_connect_cmd, check=False, capture_output=True, text=True)
                if result.returncode == 0:
                    print('[+] Access Point was saved to your network manager')
                    return True
                else:
                    print(f'[-] Failed to save network to Android network manager: {result.stderr}')
                    return False

            # Detect NetworkManager
            elif which('nmcli'):
                result = subprocess.run(networkmanager_connect_cmd, check=False, capture_output=True, text=True)
                if result.returncode == 0:
                    print('[+] Access Point was saved to your network manager')
                    return True
                else:
                    print(f'[-] Failed to save network to NetworkManager: {result.stderr}')
                    return False
            else:
                print('[-] No network manager found (nmcli not available)')
                return False

        except Exception as e:
            print(f'[-] Unexpected error while saving network: {e}')
            return False
