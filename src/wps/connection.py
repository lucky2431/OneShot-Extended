...
class Initialize:
    ...
    def __init__(self, interface: str, write_result: bool = False, save_result: bool = False, print_debug: bool = False):
        ...
        # Initialize cleanup variables first
        self.RETSOCK = None
        self.WPAS = None
        self.TEMPDIR = None
        self.TEMPCONF = None
        self.RES_SOCKET_FILE = None
        self.WPAS_CTRL_PATH = None
    ...
    def singleConnection(self, bssid: str = None, pin: str = None, pixiemode: bool = False, showpixiecmd: bool = False,
        ...
        # Check if BSSID is provided for non-PBC mode
        if not pbc_mode and bssid is None:
            print("[-] Error: BSSID not specified")
            return False
        ...
        if pbc_mode:
            success = self._wpsConnection(bssid, pbc_mode=pbc_mode)
            bssid = self.CONNECTION_STATUS.BSSID
            pin = '<PBC mode>'
        elif store_pin_on_fail:
            try:
                success = self._wpsConnection(bssid, pin, pixiemode)
                ...
                if bssid and store_pin_on_fail:
                    collector.writePin(bssid, pin)
                return False
        else:
            try:
        ...
        if self.CONNECTION_STATUS.STATUS == 'GOT_PSK':
            ...
            # WRITE_RESULT - запис у файл результатів (для відображення жовтим кольором)
            if self.WRITE_RESULT:
                print(f"[+] Записую результати у файл...")
                collector.writeResult(bssid, self.CONNECTION_STATUS.ESSID, pin, self.CONNECTION_STATUS.WPA_PSK)
            
            # SAVE_RESULT - додавання мережі до менеджера мереж
            if self.SAVE_RESULT:
                print(f"[+] Зберігаю мережу до менеджера мереж...")
                success_save = collector.addNetwork(bssid, self.CONNECTION_STATUS.ESSID, self.CONNECTION_STATUS.WPA_PSK)
                if success_save:
                    print(f"[+] ✅ Мережа успішно збережена в менеджері мереж!")
                    
                    # Додатково записуємо в stored.csv для жовтого кольору, якщо ще не записано
                    if not self.WRITE_RESULT:
                        collector.writeResult(bssid, self.CONNECTION_STATUS.ESSID, pin, self.CONNECTION_STATUS.WPA_PSK)
                else:
                    print(f"[-] Не вдалося зберегти мережу в менеджері мереж")
        ...
        if pixiemode:
            ...
            if store_pin_on_fail and bssid:
    ...
    def _initWpaSupplicant(self):
        ...
        timeout = 10  # 10 seconds timeout for wpa_supplicant startup
        ...
        while True:
            ...
            if time.time() - start_time > timeout:
                raise ValueError('wpa_supplicant took too long to start')
    ...
    def _handleWpas(self, pixiemode: bool = False, pbc_mode: bool = False, verbose: bool = None) -> bool:
        ...
        if select.select([self.WPAS.stdout], [], [], 1.0)[0]:
            ...
            # No data available, but process might still be running
    ...
    def _wpsConnection(self, bssid: str = None, pin: str = None, pixiemode: bool = False,
        ...
        # Clean the pipe - read available data to avoid old messages
        try:
            # Set non-blocking read to clear buffer
            import fcntl
            fl = fcntl.fcntl(self.WPAS.stdout, fcntl.F_GETFL)
            fcntl.fcntl(self.WPAS.stdout, fcntl.F_SETFL, fl | os.O_NONBLOCK)
            try:
                while True:
                    data = self.WPAS.stdout.read(1)
                    if not data:
                        break
            except:
                pass
            # Restore blocking mode
            fcntl.fcntl(self.WPAS.stdout, fcntl.F_SETFL, fl)
        except:
            # If non-blocking isn't supported, try regular read with timeout
            try:
                start_clear = time.time()
                while time.time() - start_clear < 2.0:  # 2 second timeout for clearing
                    if select.select([self.WPAS.stdout], [], [], 0.1)[0]:
                        data = self.WPAS.stdout.read(1024)
                        if not data:
                            break
                    else:
                        break
        ...
        while True:
            try:
                ...
                # Check timeout
                if time.time() - start_time > timeout:
                    print('[-] Timeout: WPS connection took too long')
                    break
                ...
                print(f'[-] Error in WPS connection: {e}')
                break
    ...
    def _cleanup(self):
        ...
        try:
            if hasattr(self, 'RES_SOCKET_FILE') and self.RES_SOCKET_FILE and os.path.exists(self.RES_SOCKET_FILE):
                os.remove(self.RES_SOCKET_FILE)
        except OSError:
            pass

        try:
            if hasattr(self, 'TEMPDIR') and self.TEMPDIR and os.path.exists(self.TEMPDIR):
                shutil.rmtree(self.TEMPDIR, ignore_errors=True)
        except OSError:
            pass

        try:
            if hasattr(self, 'TEMPCONF') and self.TEMPCONF and os.path.exists(self.TEMPCONF):
